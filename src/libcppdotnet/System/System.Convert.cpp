...
// From RFC 4648
std::string_view Base64Table = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
const char       Base64PadChar = '=';

std::string_view Base85Table = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstu";
const char       Base85SpecialCase1 = 'z';
const char       Base85SpecialCase2 = 'y';
const uint32_t   Base85SpecialCaseValue1 = 0u;
const uint32_t   Base85SpecialCaseValue2 = 0x20202020u; // All spaces

std::string_view Ascii85_RFC_1924 = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!#$%&()*+-;<=>?@^_`{|}~";

bool IsValidBase64(const char input)
{
    return Base64Table.find( input ) != std::string_view::npos;
}

char EncodeBase64(const uint8_t input)
{
...
std::byte DecodeBase64(char input)
{
...
bool IsValidBase85(const char input)
{
    return Base85Table.find( input ) != std::string_view::npos;
}

char EncodeBase85(const uint8_t input)
{
...
std::byte DecodeBase85(char input)
{
...
std::string From4Base64Chars(std::span<const char, 4> input)
{
...
std::string Convert::ToBase64String(std::string_view input_ascii_string)
{
